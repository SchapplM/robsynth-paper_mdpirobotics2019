...
The \todo[color=green!40]{Language}most\replaced[id=Sp]{-}{ }straightforward member of the third class is the 4U\underline{P}S-U\underline{P}U of Fig.\,\ref{fig:systems_pictures}\,b, which is investigated in \cite{MbarekNefCor2005} for a simulation and feasibility study together with a survey on possible architectures for a technical realization of this class.
...
%
%
the same way as $\bm{\beta}$ leads to a set of transformations depicted in \todo[color=red!40]{Terminology in caption}Fig.\,\ref{fig:frames_5dof_6dof}\,(a)\todo[color=green!40]{Language}\added[id=Sp]{,} where the intermediate steps of the single elementary rotations are omitted since they have no technical meaning.
...
For the case of task or kinematic redundancy, the pseudo-inverse can be used for $\bm{\Phi}_{\partial\mathrm{ap}}$ in (\ref{equ:full_diff_ik}) \todo[color=blue!40]{Clarification}\added[id=Sp]{together with a nullspace optimization} as shown in Sec.\,\ref{sec:ZB_Anwendung_IK_grad}.
\added[id=Sp]{This allows to exploit the functional redundancy in 3T2R tasks also for trajectories $\bm{\eta}(t)$, $\dot{\bm{\eta}}(t)$ and not only for single poses $\bm{\eta}$. When solving the IKP for a trajectory, the IK on position level of Sec.\,\ref{sec:ZB_Anwendung_IK_grad} is only needed to correct linearization errors.}
...
\todo[color=green!40]{Language}Therefore\added[id=Sp]{,} the 3T2R case with optimization, plotted as the green line with triangle markers, is shown only for the initial condition from Fig.\,\ref{tab:mdh_industrialrobot}.
%
\begin{figure}[b]
...
The reference trajectory can be seen at the platform position in Fig.\,\ref{fig:parrob_traj_3T2R}\,a and the platform orientation expressed in \todo[color=green!40]{Hyphenation}\replaced[id=Sp]{$XYZ$ }{$X$-$Y$-$Z$-}Euler angles ($\beta_1$-$\beta_3$) relative to the base frame in Fig.\,\ref{fig:parrob_traj_3T2R}\,b.
...
The most prominent chains are the \todo[color=green!40]{Hyphenation}UPS\replaced[id=Sp]{ }{-}chain from Sec.\,\ref{sec:Ergebnisse_IK_Parallel} at no.\,266 and the six-DoF industrial robot from Sec.\,\ref{sec:Ergebnisse_Seriell} at no.\,309.
...
This section derives the effect of the reciprocity of Euler\todo[color=green!40]{Hyphenation}\replaced[id=Sp]{ }{-}angles at the example of the kinematics description of Sec.\,\ref{sec:REW_seriell} and the frames of Fig.\,\ref{fig:frames_5dof_6dof}\,(b):
%
...
As introduced in Sec.\,\ref{sec:ZB_Anwendung}, the rotational part of the gradient matrices \todo[color=green!40]{Language}\replaced[id=Sp]{cannot}{can not} be derived with the commonly available Jacobian of the serial link kinematics of the legs.
