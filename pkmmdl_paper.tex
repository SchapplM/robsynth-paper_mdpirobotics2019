...
The second class allows more variety, since the 6-DoF mechanism and the 5-DoF constraining leg can manifest in different kinematics:
The U\underline{P}S structure is used for the 6-DoF mechanism by \cite{WangGos1997} with a focus on kinematic analysis of 5U\underline{P}S/US, by \cite{ZhangGos2001}  with a focus on kinetostatic modeling at the example of 5U\underline{P}S/RUU, by \cite{ZhengGaoZha2005} with focus on trajectory control of 5U\underline{P}S/PRPU, by \cite{GaoSunZha2004} for pose measurement with the passive leg of 5U\underline{P}S/PRPU and by \cite{ChengWanZha2008} with a redundantly actuated 5U\underline{P}S/\underline{P}RPU).
...
The most-straightforward member of the third group is the 4U\underline{P}S-U\underline{P}U, which is investigated in \cite{LinLiYanZha2013} regarding the inverse dynamics or in \cite{MbarekNefCor2005} for a simulation and feasibility study.
A survey on possible architectures for a technical realization is also given by \cite{MbarekNefCor2005}.
...
These findings can be applied to the Metrom machine tool (5S\underline{P}RR), which is analyzed in \cite{BaerWei2006} or its variant, the redundant 4S\underline{P}RR-\underline{P}S\underline{P}R from \cite{AlaghebandMahMilBen2015}, where the last joint axis of each leg is coaxial with the tool axis and is constructed as rotating ring.
A structural synthesis based on linear transformations and evolutionary morphology \cite{Gogu2008} led to e.\,g. to the Isoglide5 mechanisms (3\underline{P}RRRRR-2\underline{P}RRRR), which are analyzed and optimized regarding the isotropy of the Jacobian in \cite{Gogu2006}.

The last group consists of symmetric machines, which have been found by systematic structural synthesis, which has been performed in \cite{KongGos2005} for 3T2R with screw theory.
...
Therefore members of the classes I and IV can be expected to reach a higher stiffness, therefore justifying attempts of this paper.
...
The parallel robots with five DoF presented above have a kinematic structure, that allows an analytic model of the inverse kinematics.
...
To the best knowledge of the authors, only one reference for the IKP of functionally redundant parallel robots of the class I is known.
The assumed reason is that a solution of the 3T3R IKP for these robots is possible with standard methods, as used in \cite{HongKim2000}.
It is always possible to transfer the 3T2R IK into 3T3R by adding an arbitrary value for the desired rotation around the tool axis.
...
To select the best machine for given requirements, a kinematic structural synthesis is only the first step.
...
\begin{itemize}
\item a novel general kinematics model for parallel robots in 3T2R tasks using the concept of reciprocal Euler angles \cite{1_SchapplerTapOrt2019} and allowing nullspace optimization in the gradient-based inverse kinematics,
\item a complete elimination of the redundant operational space coordinate,
\item examples and simulations to show the performance for serial and parallel robots.
\end{itemize}
...
This is termed ``minimal kinematics set'' in \cite{Merlet} and leads to the scalar constraint equation
%
\begin{equation}
\Phi_{i}
=
...
\begin{enumerate}
    ...
    This issue is solved by the theory of linear transformations used by \cite{Gogu2008}.
