...
Parallel robots in 3T2R tasks can be ordered in classes according their task redundancy and symmetry \cite{Tale-MasoulehGos2011}:
\begin{itemize}
...
Fig.\,\ref{fig:pkm_constr_rot_lead}\,(b) and (d) show cases, where the translational constraints are met, but the rotational constraints have different values.
For 3T3R tasks, only Fig.\,\ref{fig:pkm_constr_rot_lead}\,(d) represents a valid solution of the inverse kinematics. For 3T2R tasks, Fig.\,\ref{fig:pkm_constr_rot_lead}\,(b) and (d) represent valid solutions.
...
The definition of the full kinematic constraints (\ref{equ:Phi_def}) of a single leg chain of the parallel robot from the previous chapter can be used to write the kinematic constraints
...
\end{equation}
%
where the translational component can be calculated with the geometric Jacobian of the leg chain, as derived in appendix\,\ref{sec:appendix_gradient_geomjacobian}. % $\bm{J}_1=\begin{pmatrix}\bm{J}_{1,\mathrm{t}}^\transp & \bm{J}_{1,\mathrm{r}}^\transp\end{pmatrix}^\transp$
...
As explained regarding (\ref{equ:constr_Phi_PKM}), the constraints (\ref{equ:Phit_def_i}) and (\ref{equ:Phir_def_i}) and their gradients (\ref{equ:grad_Phi_1_q}) and (\ref{equ:grad_Phi_1_x}) are used for the leading leg.
For the following legs, the gradients $\bm{\Phi}_{j,\partial\bm{q}_1}$, $\bm{\Phi}_{j,\partial\bm{q}_j}$ and $\bm{\Phi}_{j,\partial\bm{x}}$ from (\ref{equ:PhiPsi_grad_q_complete}) and (\ref{equ:PhiPsi_grad_x_complete}) have to be calculated in a similar way. 
