...
Deviations in the parameters relative to \cite{HuoBar2008} result from the use of the \emph{modified} \todo[color=blue!40]{DH footnote removed} Denavit-Hartenberg notation \replaced[id=Sp]{from \cite{BriotKha2015} for the joint transformations}{for the joint transformation according to \textsc{Khalil}} and the use of only positive \todo[color=green!40]{Language} \replaced[id=Sp]{axis alignment parameters $\alpha_i$}{$\alpha_i$ parameters for axis alignment}
...
\todo[color=green!40]{Language}Additionally, the Denavit\added[id=Sp]{-}Hartenberg parameters were set to 50 different sets of uniformly distributed parameters between $0$ and $1$ meters or radians resulting to 2500 combinations for each of the 309 chains in total.
...
\end{equation}
%
and the \todo[color=green!40]{Terminology} \replaced[id=Sp]{reciprocal}{inverse} gradient ``IV'' in (\ref{equ:grad_Phi_1_x}) for $X$-$Y$-$Z$ angles yields
%
