...
\end{equation}
%
for the following leg is given by the Euler angle representation of the deviation between the orientation of the platform frame $\ks{L}$ given by the leading leg and the frame $\ks{E}$ given by the respective following leg.
...
The gradient matrices $\bm{\Phi}_{\partial \bm{q}}$ and $\bm{\Phi}_{\partial \bm{x}}$ contain nested nonlinear functions related to the orientation error, therefore the geometric Jacobian of the leg chains can not be exploited for the rotational part.
...
=
&=
\frac{\partial}{\partial \bm{x}_{\mathrm{r}}} \bm{\alpha}\left(\left(\rotmat{0}{E}^\transp(\bm{q}_i) \rotmat{0}{D}(\bm{x}_{\mathrm{r}})\right)^\transp\right) \label{equ:grad_Phi_1_x} 
=
...
As explained regarding (\ref{equ:constr_Phi_PKM}), the constraints (\ref{equ:Phit_def_i}), (\ref{equ:Phir_def_i}) and their gradients (\ref{equ:grad_Phi_1_q}), (\ref{equ:grad_Phi_1_x}) already allow the kinematic modeling of 3T3R robots.
...
\end{equation}
%
The gradient w.\,r.\,t. the joint coordinates of the following leg
%
\begin{align}
...
\underbrace{\left(\frac{\partial \overline{\prod}\left(\rotmato{0}{D}^\transp, \rotmato{0}{E}\right)}{\partial \rotmato{0}{E}}\right)}_{\mathrm{II} \in {\mathbb{R}}^{9 \times 9}}
% Term III
\underbrace{\vphantom{\left(\frac{\partial \overline{\prod}\left(\rotmato{0}{D}^\transp, \rotmato{0}{E}\right)}{\partial \rotmato{0}{E}}\right)}\left(\frac{\partial \rotmato{0}{E}(\bm{q}_j)}{\partial \bm{q}_j} \right)}_{\mathrm{III} \in {\mathbb{R}}^{9 \times \mathrm{dim}(\bm{q}_j)}}.  \nonumber
\end{align}
%
is similar to the expression in (\ref{equ:grad_Phi_1_q}). The gradient w.\,r.\,t. the joint coordinates of the leading leg
%
\begin{align}
...
%underbrace für Geschweifte Klammern drunter, vphantom nur für vertikale Größe des mittleren Terms
% Term I:
\underbrace{\vphantom{\left(\frac{\partial \overline{\prod}\left(\rotmato{0}{E}^\transp, \rotmato{0}{L}\right)}{\partial \rotmato{0}{L}}\right)}\left(\frac{\partial \bm{\alpha}}{\partial \overline{\bm{R}}}\right)}_{\mathrm{I} \in {\mathbb{R}}^{3 \times 9}}
% Term II:
\underbrace{\left(\frac{\partial \overline{\prod}\left(\rotmato{0}{E}^\transp, \rotmato{0}{L}\right)}{\partial \rotmato{0}{L}}\right)}_{\mathrm{II} \in {\mathbb{R}}^{9 \times 9}}
% Term III:
\underbrace{\vphantom{\left(\frac{\partial \overline{\prod}\left(\rotmato{0}{D}^\transp, \rotmato{0}{E}\right)}{\partial \rotmato{0}{E}}\right)}\left(\frac{\partial \rotmato{0}{L}(\bm{q}_1)}{\partial \bm{q}_1} \right)}_{\mathrm{III} \in {\mathbb{R}}^{9 \times \mathrm{dim}(\bm{q}_1)}}.  \nonumber
\end{align}
%
