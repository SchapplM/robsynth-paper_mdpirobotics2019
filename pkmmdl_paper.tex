...
First, the simple optimization criterion $h_1$ from (\ref{equ:criterion1}) showed violations of the limits of one joint which were compensated by improvements for other joints.
...
To emphasize the generality of the approach for solving the IKP, the inverse kinematics is solved for a set of 309 serial kinematic chains with six joints generated by permutations on the Denavit-Hartenberg parameters that describe the robot kinematics and is reduced with the isomorphism detection of \cite{RamirezKotOrt2015} to a minimal set.
In contrast to the evaluation in the previous section, the inverse kinematics is solved for arbitrary reachable poses of the serial chain that are generated by the forward kinematics of 50 different joint configurations of the chains uniformly distributed between the joint limits of $\pm \pi$ for rotational joints and $\pm0.5\,\mathrm{m}$ for prismatic joints.
Additionally the Denavit Hartenberg parameters (distances between joints and angle offsets) were set to 50 different sets of uniformly distributed parameters between $0$ and $1$ resulting to 2500 combinations for each of the 309 robots in total.
The inital value $\bm{q}^0$ for the solution of the IKP was set to random values from a uniform distribution within the joint limits.
The results are presented in Fig.\,\ref{fig:serrob_ik_hist} for different settings of the algorithm.
A maximum of 15 tries with random initial values was allowed to search for a solution of the IKP within the limits.
After that, five more tries were allowed to find a solution violating the limits, but presenting a solution of the IKP.
Fig.\,\ref{fig:serrob_ik_hist}\,a shows the distribution of the results for 
...
\begin{figure}[tb]
	...
	\caption{Results of the inverse kinematics of a 6UPS robot in a 3T2R task}
