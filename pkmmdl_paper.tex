...
The overview over the literature shows, that no general, \todo[color=green!40]{Language}machine-independent method\deleted[id=Sp]{s} for the resolution of functional redundancy for 3T3R PKM in 3T2R tasks exists.
...
%
%
the same way as $\bm{\beta}$ leads to a set of transformations depicted in \todo[color=red!40]{Terminology in caption}Fig.\,\ref{fig:frames_5dof_6dof}\,(a) where the intermediate steps of the single elementary rotations are omitted since they have no technical meaning.
...
The \todo[color=green!40]{Language}\replaced[id=Sp]{reasoning}{reasonings} so far only considered the inverse kinematics of the parallel robot.
