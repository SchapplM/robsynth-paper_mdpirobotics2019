...
%
\begin{equation}
\bm{\varPhi}_{\mathrm{t},i}(\bm{q}_i,\bm{x}) = - \ortvek{0}{r}{}{A_iB_i}(\bm{x}) + \ortvek{0}{r}{}{A_iB_i}(\bm{q}_i) 
...
%
\begin{equation}
\frac{\mathrm{d}}{{\mathrm{d}}t} \bm{\varPhi}(\bm{q}_\mathrm{a},\bm{x})
=
\bm{\varPhi}_{\partial \bm{q}_\mathrm{a}}  \dot{\bm{q}}_\mathrm{a}
+
\bm{\varPhi}_{\partial \bm{x}} \dot{\bm{x}}
=
\bm{0}
...
%
\begin{equation}
\bm{\varPhi}_{\partial \bm{q}_{\mathrm{a}}}
=
\frac{\partial \bm{\varPhi}}{\partial \bm{q}_{\mathrm{a}}}
=
\begin{bmatrix}
\varPhi_{1,\partial \bm{q}_{1,\mathrm{a}}}  & 0 & 0 & 0\\
0 & \varPhi_{2,\partial \bm{q}_{2,\mathrm{a}}} & \ddots & 0  \\
0 & \ddots & \ddots & 0  \\
0 & 0 & 0 & \varPhi_{m,\partial \bm{q}_{m,\mathrm{a}}}
\end{bmatrix}
...
%
\begin{equation}
\bm{\varPhi}_{\partial \bm{x}}
=
\frac{\partial \bm{\varPhi}}{\partial \bm{x}}
=
\begin{bmatrix}
\partial \varPhi_1/\partial \bm{x}\\
\partial \varPhi_2/\partial \bm{x}\\
\vdots \\
\partial \varPhi_m/\partial \bm{x}\\
\end{bmatrix}
...
%
%\begin{equation}
%\frac{\partial \bm{\varPhi}_i}{\partial \bm{q}_i}
%=
%\begin{bmatrix}
%* & \cdots & * & 0 & \cdots & 0\\
...
%
\begin{equation}
\bm{\varPhi}_{\mathrm{t},i}(\bm{q}_i,\bm{x})
= 
...
%
\begin{equation}
\bm{\varPhi}_i=\begin{bmatrix}
\bm{\varPhi}_{\mathrm{t},i}^\transp & \bm{\varPhi}_{\mathrm{r},i}^\transp
...
%
\begin{align}
\bm{\varPhi}_{\mathrm{r},i}(\bm{q}_i,\bm{x})
=
\begin{bmatrix}
...
Similar to the definition of the rotational operational space coordinates $\bm{x}_\mathrm{r}$ in (\ref{equ:xr_def}), the constraints $\bm{\varPhi}_{\mathrm{r},i}$ are also expressed with a set of Euler angles $\bm{\alpha}$.
...
%
\begin{equation}
\bm{\varPsi}_{\mathrm{r},i}(\bm{q}_i,\bm{\eta}) = 
\begin{bmatrix}
\alpha_2  & \alpha_3
\end{bmatrix}^\transp
=
\overbrace{\begin{bmatrix}
    0 & 1 & 0  \\ 
    0 & 0 & 1
    \end{bmatrix}}^{=\bm{P}_{\bm{\varPsi}_\mathrm{r}}}
\bm{\varPhi}_{\mathrm{r},i} (\bm{q}_i,\bm{x})
\in {\mathbb{R}}^{2}
\label{equ:Psir_def}
\end{equation}
...
%
\begin{equation}
\bm{\varPsi}_i=\begin{bmatrix}
\bm{\varPhi}_{\mathrm{t},i}^\transp & \bm{\varPsi}_{\mathrm{r},i}^\transp
\end{bmatrix}^\transp \in {\mathbb{R}}^{5}
\label{equ:Psi_def}
\end{equation}
%
can be used for the inverse kinematics of the leg chain $i$ in 3T2R tasks, where $\bm{\varPsi}_i=\bm{0}$ leads to a valid position and orientation of the tool axis.
The condition $\bm{\varPhi}_i=\bm{0}$ leads to a valid configuration of leg $i$ of the parallel robot in 3T3R tasks.
In the following, ``$\bm{\varPhi}$'' is always used for 3T3R\footnote{By omitting the corresponding lines in the operational space coordinates $\bm{x}$ and the constraint equations $\bm{\varPhi}$, it is also possible to use the 3T3R approach for systems with reduced mobility of 2T1R, 3T0R and 3T1R platform DoF.} kinematic descriptions and ``$\bm{\varPsi}$'' for 3T2R.
...
\end{equation}
%
The constraints $\bm{\varPsi}_i$ from (\ref{equ:Psi_def}) for the reduced coordinates $\bm{\eta}$ can only be defined for one leg chain:
...
\end{figure} 
%
By also closing the \emph{3T2R} kinematic constraints $\bm{\varPsi}_2$ for the second loop, as depicted in Fig.\,\ref{fig:pkm_constr_rot_follow}\,(b), the tool axis stays arbitrary and the platform pose demanded from the two legs would be different and therefore would not be a valid solution for the complete mechanism, i.\,e. $\bm{\varPhi}_2 \ne \bm{0}$.
...
%
\begin{equation}
\bm{\varPhi}_{\mathrm{t},j}(\bm{q}_j,\bm{x})
= 
...
%
\begin{equation}
\bm{\varPhi}_{\mathrm{r},j}(\bm{q}_j,\bm{q}_1)
=
...
\end{equation}
%
are assembled from the 3T2R constraints $\bm{\varPsi}_1$ from (\ref{equ:Psi_def}) for the leading leg and the 3T3R constraints $\bm{\varPhi}_j$, $2\le{}j\le{}m$ from (\ref{equ:Phit_def_j},\ref{equ:Phir_def_j}) for the following legs.
...
To be able to compute the differential kinematics of the constraints $\bm{\varPhi}$ (\ref{equ:constr_Phi_PKM}) and  $\bm{\varPsi}$ (\ref{equ:constr_Psi_PKM}) to
%
\begin{equation}
\frac{\mathrm{d}}{{\mathrm{d}}t} \bm{\varPhi}(\bm{q},\bm{x})
=
\bm{\varPhi}_{\partial \bm{q}}  \dot{\bm{q}}
+
\bm{\varPhi}_{\partial \bm{x}} \dot{\bm{x}}
=
\bm{0}
...
\mathrm{and}
\quad
\frac{\mathrm{d}}{{\mathrm{d}}t} \bm{\varPsi}(\bm{q},\bm{\eta})
=
\bm{\varPsi}_{\partial \bm{q}}  \dot{\bm{q}}
+
\bm{\varPsi}_{\partial \bm{\eta}} \dot{\bm{\eta}}
=
\bm{0},
...
%
\begin{equation}
\bm{\varPhi}_{\partial \bm{q}}(\bm{q},\bm{x})
%=
%\frac{\partial \bm{\varPhi}}{\partial \bm{q}}
=
\begin{bmatrix}
\bm{\varPhi}_{1,\partial\bm{q}_1}  & \bm{0} & \bm{0} & \bm{0}\\
\bm{0} & \bm{\varPhi}_{2,\partial\bm{q}_2} & \ddots & \bm{0}  \\
\vdots & \ddots & \ddots & \bm{0}  \\
\bm{0} & \bm{0} & \bm{0} &\bm{\varPhi}_{m,\partial\bm{q}_m}
\end{bmatrix}
\quad
\mathrm{and}
\quad
\bm{\varPsi}_{\partial \bm{q}}(\bm{q},\bm{\eta})
=
\begin{bmatrix}
\bm{\varPsi}_{1,\partial\bm{q}_1}  & \bm{0} & \bm{0} & \bm{0}\\
\bm{\varPhi}_{2,\partial\bm{q}_1} & \bm{\varPhi}_{2,\partial\bm{q}_2} & \ddots & \bm{0}  \\
\vdots & \ddots & \ddots & \bm{0}  \\
\bm{\varPhi}_{m,\partial\bm{q}_1} & \bm{0} & \bm{0} &\bm{\varPhi}_{m,\partial\bm{q}_m}
\end{bmatrix}
...
%
\begin{equation}
\bm{\varPhi}_{\partial \bm{x}}(\bm{q},\bm{x})
=
\frac{\partial \bm{\varPhi}}{\partial \bm{x}}
=
\begin{bmatrix}
\bm{\varPhi}_{1,\partial\bm{x}} \\
\bm{\varPhi}_{2,\partial\bm{x}} \\
\vdots \\
\bm{\varPhi}_{m,\partial\bm{x}}
\end{bmatrix}
\quad
\mathrm{and}
\quad
\bm{\varPsi}_{\partial \bm{\eta}}(\bm{q},\bm{\eta})
=
\frac{\partial \bm{\varPsi}}{\partial \bm{\eta}}
=
\begin{bmatrix}
\bm{\varPsi}_{1,\partial\bm{\eta}} \\
\bm{\varPhi}_{2,\partial\bm{\eta}} \\
\vdots \\
\bm{\varPhi}_{m,\partial\bm{\eta}} \\
\end{bmatrix}
\label{equ:PhiPsi_grad_x_complete}
\end{equation}
%
have to be calculated for the 3T3R and the 3T2R case respectively.
The gradient $\bm{\varPhi}_{\partial \bm{q}}$ has block diagonal form, indicating that the inverse kinematics problem can be solved for each leg independently.
The structures of the gradient $\bm{\varPsi}_{\partial \bm{q}}$ results from the coupling of the leading and following joints in the rotational constraints equation.
%\color{gray}
%Since the 3T2R case can be inferred from the 3T3R case by the definition of reciprocal Euler angles and the selection of the corresponding entries of the coordinates in (\ref{equ:etar_def}) and the constraints in (\ref{equ:Psir_def}), only the 3T3R case ``$\bm{\varPhi}$'' is written out in the following elaborations.
%\color{black} % Stimmt so gar nicht mehr.

The gradient matrices $\bm{\varPhi}_{\partial \bm{q}}$ and $\bm{\varPhi}_{\partial \bm{x}}$ contain nested nonlinear functions related to the orientation error, therefore the geometric Jacobian of the leg chains can not be exploited for the rotational part, as derived in appendix\,\ref{sec:appendix_gradient_geomjacobian}.
...
%
The constraint definition $\bm{\varPsi}_1$ for the leading leg of the 3T2R case (\ref{equ:constr_Psi_PKM}) and $\bm{\varPhi}_i$ with $i=1,...,m$ for all legs of the 3T3R case (\ref{equ:constr_Phi_PKM}) are subject to the same model of (\ref{equ:Phit_def_i},\ref{equ:Phir_def_i},\ref{equ:Phi_def}).
In the following, the 3T3R constraints are displayed.
The form $\bm{\varPsi}_1$ for the 3T2R case is obtained by removing the corresponding line of the rotational component according to (\ref{equ:Psir_def}) and replacing ``$\bm{\varPhi}$'' by ``$\bm{\varPsi}$'' in the following equations.
For the analysis, the constraint gradient matrix w.\,r.\,t. the joint coordinates has to be divided out to
%
\begin{equation}
\bm{\varPhi}_{1,\partial\bm{q}_1}
=
\begin{bmatrix}
\bm{\varPhi}_{\mathrm{t},1,\partial\bm{q}_1}^\transp
&
\bm{\varPhi}_{\mathrm{r},1,\partial\bm{q}_1}^\transp
\end{bmatrix}^\transp,
...
\begin{equation}
\bm{\varPhi}_{\mathrm{r},1,\partial\bm{q}_1}
=
...
=
\begin{bmatrix}
\bm{\varPhi}_{\mathrm{t},1,\partial\bm{x}_\mathrm{t}} & \bm{\varPhi}_{\mathrm{t},1,\partial\bm{x}_\mathrm{r}} \\
\bm{\varPhi}_{\mathrm{r},1,\partial\bm{x}_\mathrm{t}} &
\bm{\varPhi}_{\mathrm{r},1,\partial\bm{x}_\mathrm{r}}
\end{bmatrix}
=
\begin{bmatrix}
-\bm{1} & \bm{0} \\
...
For the following legs in the 3T2R case, the gradients $\bm{\varPhi}_{j,\partial\bm{q}_1}$, $\bm{\varPhi}_{j,\partial\bm{q}_j}$ and $\bm{\varPhi}_{j,\partial\bm{x}}$ with $j=2,...,m$ from the right part of  (\ref{equ:PhiPsi_grad_q_complete}) and (\ref{equ:PhiPsi_grad_x_complete}) have to be calculated in a similar way. 
...
\end{equation}
%
The gradient w.\,r.\,t. the joint coordinates of the following leg contains again the translational part of the legs Jacobian regarding the end-effector platform position in $\bm{\varPhi}_{\mathrm{t},j,\partial\bm{q}_j}$ and has the rotational part
%
\begin{align}
\bm{\varPhi}_{\mathrm{r},j,\partial\bm{q}_j}
&=
...
%
\begin{align}
\bm{\varPhi}_{\mathrm{r},j,\partial\bm{q}_1}
&=
...
As first introduced in \cite{GoldenbergBenFen1985} for Euler angle residuals in the IKP, the Taylor series expansion of $\bm{\varPhi}(\bm{q},\bm{x})$ leads to the definition of
%
\begin{equation}
\bm{\varPhi}(\bm{q}^{k+1},\bm{x}) = 
\bm{\varPhi}(\bm{q}^{k},\bm{x})
+
\frac{\partial}{\partial \bm{q}} \bm{\varPhi}(\bm{q},\bm{x}) \biggr\rvert_{\bm{q}^k} (\bm{q}^{k+1} - \bm{q}^k)
...
\bm{q}^{k+1} - \bm{q}^k
=
-\bm{\varPhi}_{\partial\bm{q}}^{\dagger}(\bm{q}^k,\bm{x}) \bm{\varPhi}(\bm{q}^k,\bm{x}).
\label{equ:IKP_maintask}
\end{equation}
%
Depending on the dimension, $(\cdot)^\dagger$ denotes the matrix inverse or the pseudo-inverse.
Again, the equations (\ref{equ:taylor_phi}-\ref{equ:IKP_maintask}) can be written with ``$\bm{\varPsi}$'' from (\ref{equ:constr_Psi_PKM}) instead of ``$\bm{\varPhi}$'' from (\ref{equ:constr_Phi_PKM}) for the 3T2R case.

In the latter case, the corresponding gradient matrix $\bm{\varPsi}_{\partial \bm{q}}(\bm{q},\bm{\eta})$ from (\ref{equ:PhiPsi_grad_q_complete}) allows the definition of a nullspace in the case of $\mathrm{dim}(\bm{q}_1)>\mathrm{dim}(\bm{\eta})$.
...
=
-\bm{\varPsi}_{\partial\bm{q}}^{\dagger} \bm{\varPsi} +  (\bm{1}-\bm{\varPsi}_{\partial\bm{q}}^{\dagger}\bm{\varPsi}_{\partial\bm{q}}) h_{\partial\bm{q}}
\label{equ:nullspace}
\end{equation}
%
...
The platform pose $\bm{x}$/$\bm{\eta}$ corresponds to the desired pose for the serial robots end-effector and the kinematic constraints $\bm{\varPhi}$/$\bm{\varPsi}$ correspond to the residual of the IKP.
...
Further, using $\bm{\varPsi}$/$\bm{\eta}$ as elaborated before allows for the first time to define differential kinematics specifically for 3T2R tasks in a general form.
The differential equation of (\ref{equ:constr_diff}) is expanded to 
\begin{equation}
\frac{\mathrm{d}}{\mathrm{d}t} \bm{\varPhi}(\bm{q}_{\mathrm{a}}, \bm{q}_{\mathrm{p}},\bm{x})
=
\bm{\varPhi}_{\partial\bm{q}_{\mathrm{a}}} \dot{\bm{q}}_{\mathrm{a}} + \bm{\varPhi}_{\partial\bm{q}_{\mathrm{p}}} \dot{\bm{q}}_{\mathrm{p}} + \bm{\varPhi}_{\partial\bm{x}} \dot{\bm{x}} 
=
\bm{0}
...
=
- 
\begin{bmatrix} \bm{\varPhi}_{\partial\bm{q}_{\mathrm{a}}} & \bm{\varPhi}_{\partial\bm{q}_{\mathrm{p}}}\end{bmatrix}
\begin{bmatrix}
\dot{\bm{q}}_{\mathrm{a}} \\
\dot{\bm{q}}_{\mathrm{p}} \\
\end{bmatrix}
=
\bm{\varPhi}_{\partial\mathrm{ap}}
\begin{bmatrix}
\dot{\bm{q}}_{\mathrm{a}} \\
\dot{\bm{q}}_{\mathrm{p}} \\
\end{bmatrix},
...
\end{bmatrix}
=
-\bm{\varPhi}_{\partial\mathrm{ap}}^{-1} \bm{\varPhi}_{\partial\bm{x}}
\dot{\bm{x}},
...
=
- 
\begin{bmatrix} \bm{\varPhi}_{\partial\bm{x}} & \bm{\varPhi}_{\partial\bm{q}_{\mathrm{p}}} \end{bmatrix}
\begin{bmatrix}
\dot{\bm{x}} \\
\dot{\bm{q}}_{\mathrm{p}}
\end{bmatrix}
=
\bm{\varPhi}_{\partial\bm{x}\mathrm{p}}
\begin{bmatrix}
\dot{\bm{x}} \\
\dot{\bm{q}}_{\mathrm{p}}
\end{bmatrix},
...
\end{bmatrix}
=
-\bm{\varPhi}_{\partial\bm{x}\mathrm{p}}^{-1} \bm{\varPhi}_{\partial\bm{q}_{\mathrm{a}}}
\dot{\bm{q}}_{\mathrm{a}}.
...
For the case of task or kinematic redundancy, the pseudo-inverse can be used for $\bm{\varPhi}_{\partial\mathrm{ap}}$ in (\ref{equ:full_diff_ik}) as shown in Sec.\,\ref{sec:ZB_Anwendung_IK_grad}.
...
%For meaningful PKM the matrix $\bm{\varPhi}_{\partial\bm{x}\mathrm{p}}$ is always square, since otherwise one or more leg chains would contain kinematic redundancy leading to uncontrollable nullspace motion. --> ist so noch falsch/ungeprüft!
...
%
\begin{equation}
\frac{\mathrm{d}}{\mathrm{d}t} \bm{\varPhi}_1(\bm{q}_1,\bm{x})
=
\bm{\varPhi}_{1,\bm{q}}(\bm{q}_1,\bm{x}) \dot{\bm{q}}_1 + \bm{\varPhi}_{1,\bm{x}}(\bm{q}_1,\bm{x}) \dot{\bm{x}} 
=
\bm{0}
...
\begin{bmatrix}
-\bm{1} & \bm{0} \\
\bm{0} & \bm{\varPhi}_{\mathrm{r},1,\partial\bm{x}_\mathrm{r}}^{-1}
\end{bmatrix}
\begin{bmatrix}
\bm{\varPhi}_{\mathrm{t},1,\partial\bm{q}_1}
&
\bm{\varPhi}_{\mathrm{r},1,\partial\bm{q}_1}
\end{bmatrix}
\dot{\bm{q}}_1
=
\begin{bmatrix}
\bm{\varPhi}_{\mathrm{t},1,\partial\bm{q}_1} & \bm{0} \\
\bm{0} & -\bm{\varPhi}_{\mathrm{r},1,\partial\bm{x}_\mathrm{r}}^{-1} \bm{\varPhi}_{\mathrm{r},1,\partial\bm{q}_1}
\end{bmatrix}
\dot{\bm{q}}_1.
...
%
\begin{align}
\bm{\varPhi}_{1,\bm{q}}(\bm{q}_1)
&=
-\bm{\varPhi}_{1,\bm{x}}(\bm{q}_1,\bm{x})
\bm{J}_1(\bm{q}_1),
\label{equ:KoeffVgl_Phi1d1}
\\
\bm{\varPhi}_{\mathrm{t},1,\partial\bm{q}_1}(\bm{q}_1)
&=
\bm{J}_{\mathrm{t},1}(\bm{q}_1)\quad\mathrm{and}
\label{equ:KoeffVgl_Phi1td1}
\\
\bm{\varPhi}_{\mathrm{r},1,\partial\bm{q}_1}(\bm{q}_1,\bm{x})
&=
-\bm{\varPhi}_{\mathrm{r},1,\partial\bm{x}_\mathrm{r}}(\bm{q}_1,\bm{x}) \bm{J}_{\mathrm{r},1}(\bm{q}_1)
...
%Since the term $\bm{\varPhi}_{\mathrm{r},1,\partial\bm{x}_\mathrm{r}}$ depends on $\bm{q}_1$ \emph{and} $\bm{x}$
