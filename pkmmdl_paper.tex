...
This paper presents the definition of the rotational component of kinematic constraints of parallel robots based on two reciprocal sets of Euler angles for the end-effector orientation and the orientation residual.
...
Parallel robots in 3T2R tasks can be ordered in groups according their task redundancy and symmetry \cite{Tale-MasoulehGos2011}:
\begin{itemize}
	\item[I] mechanisms with full platform mobility (3T2R) that are redundantly controlled to five DoF,
...
Examples for the first class are hexapods\footnote{The joint structure is denoted by the order of universal (``U''), prismatic (``P''), spherical (``S'') and revolute (``R'') joints in the leg chains. Different actuated legs are connected by dashes (``-''), passive constraint legs are connected by a slash (``/''). Actuated joints are underlined.} (6U\underline{P}S) \cite{MerletPerDan2000} or the Eclipse machine tool (2\underline{PPR}S-\underline{PP}RS) \cite{HongKim2000}.
...
The most-straightforward member of the third group is the 4U\underline{P}S-U\underline{P}U, which is investigated in \cite{MbarekNefCor2005} for a simulation and feasibility study together with a survey on possible architectures for a technical realization of this class.
...
These findings can be applied to the Metrom machine tool (4S\underline{P}RR-S\underline{P}R), which is analyzed in \cite{BaerWei2006} or its variants, the redundant 4S\underline{P}RR-\underline{P}S\underline{P}R from \cite{AlaghebandMahMilBen2015} or the hybrid 4U\underline{P}S-U\underline{P}U\footnote{The machine is termed ``4URHU-1URHR'' by the authors, where the rotational part of the helical joints is actuated.} with an additional linear actuator at the platform \cite{LinLiYanZha2013}.
The last joint axis of the legs of this sub-class is coaxial with the tool axis and is constructed as rotating ring.
...
The last group (IV) consists of symmetric machines, which have been found by systematic structural synthesis, which has been performed in \cite{KongGos2005} for 3T2R with screw theory.
...
In a practical application with competing requirements on workspace, stiffness, costs and precision, each of the existing systems has their legitimation.
The remainder of the paper focuses on mechanisms of the first class to allow to optimize their performance criteria using the degree of task redundancy.
In tasks like milling with high process forces and requirements on stiffness and precision, robots from class II with a constraining leg have the drawback, that the constraining leg takes the complete reaction wrench in the blocked rotational degree of freedom, which strongly affects the mechanisms stiffness in this direction \cite{ZhangGos2001}.
The same is argued by \cite{LinLiYanZha2013} at the example of the Metrom machine tool, but can be extended to most members of class III, where most leg chains contain six DoF and only one leg chain has five DoF.
...
Additionally, only members of class I provide redundancy which allows to use the additional DoF for performance optimizations e.\,g. to avoid singularities.
...
For robots of class II, the constraining leg is selected as the leading leg chain. %  \cite{WangGos1997}
...
The assumed reason is that a solution of the 3T3R IKP for these robots is possible with standard methods, as used in \cite{HongKim2000} for the 3T3R Eclipse.
...
The drawback of this method is the need for a cascaded optimization which is more complex than a gradient-based approach.
...
\item a complete elimination of the redundant operational space coordinate in this formulation for 3T2R tasks leading to a nullspace optimization in the gradient-based inverse kinematics,
...
\begin{enumerate}
    \item For parallel robots with arbitrary leg chains like those generated by a structural synthesis \cite{KongGos2005, Gogu2008, RamirezKotOrt2015}, it is generally not possible to eliminate the passive joint coordinates.
    If more than three joints per leg influence the coupling point position $B_i$, the three kinematic constraints per joint in (\ref{equ:kinconstrAB}) are not sufficient to generate enough equations for the matrix of (\ref{equ:PKM_phi_grad_q}) to become invertible.
    Projection-based methods for the derivation of the dynamics \cite{Merlet2006,BriotKha2015,AbdellatifHei2009,DoThanhKotHeiOrt2009b} underly the same restrictions.
    The velocity based theory of linear transformations used by \cite{Gogu2008} allows to determine the mobility of parallel robots, but does not provide a direct mean to solve the IKP, since this requires a formulation of the orientation at position level.
...
\end{enumerate}
%
An alternative kinematic model to encounter the combination of the two points is presented in the next section.
...
\begin{figure}[tb]
    ...
    \caption{Different cases for the kinematic constraints of the leading chain for 3RRR example: (a) no constraints complied; (b) position and rotation about tool axis complied. Different orientations in 3D are assumed to be complied; (c) wrong position, but correct orientation; (d) all constraints complied.}
...
The vector loop is depicted in Fig.\,\ref{fig:pkm_constr_rot_lead}\,(a) and (d) for a planar robot with opened and closed loops.
...
Fig.\,\ref{fig:pkm_constr_rot_lead}\,(b) and (d) shows cases, where the translational constraints are met, but the rotational constraints have different values. For 3T3R tasks, only Fig.\,\ref{fig:pkm_constr_rot_lead}\,(d) represents a valid solution of the inverse kinematics. For 3T2R tasks, Fig.\,\ref{fig:pkm_constr_rot_lead}\,(b) and (d) represent valid solutions.
...
By also closing the \emph{3T2R} kinematic constraints for the second loop, as depicted in Fig.\ref{fig:pkm_constr_rot_follow}\,(b), the tool axis stays arbitrary and the platform pose demanded from the two legs would be different and therefore would not be a valid solution for the complete mechanism.
...
%%
%\begin{equation}
%\frac{\mathrm{p}}{{\mathrm{p}}t} \bm{\Phi}(\bm{q},\bm{x})
%=
...
(\ref{equ:alpha_xyz_diff_R}) with $\overline{\bm{R}}=\rotmato{L}{E}(\bm{q}_1,\bm{q}_j)$ in ``I'',
the permutation matrix for transposition from (\ref{equ:transposition_operator}) in ``II'',
(\ref{equ:diff_prod_mat2}) in ``III'' and
...
Based on the inverse kinematics using Euler angle based residuals in \cite{GoldenbergBenFen1985}, the Taylor series expansion of $\bm{\Phi}(\bm{q},\bm{x})$ leads to the definition of
%
\begin{equation}
...
\end{equation}
Using $\bm{\Psi}$ for 3T2R tasks from (\ref{equ:constr_Psi_PKM}) instead of $\bm{\Phi}$ from (\ref{equ:PhiPsi_grad_q_complete}) and the corresponding gradient matrix from (\ref{equ:PhiPsi_grad_q_complete}) allows the definition of a Nullspace in the gradient matrix $\bm{\Psi}_{\partial \bm{q}}(\bm{q},\bm{\eta})$ in the case of $\mathrm{dim}(\bm{q}_1)>\mathrm{dim}(\bm{\eta})$.
The Nullspace can be exploited by using the Nullspace projection ${\Delta}\bm{q}_{\mathrm{N}}$ additionally to the solution ${\Delta}\bm{q}_{\mathrm{T}}$ of the IKP with the increment
%
\begin{equation}
{\Delta}\bm{q}
=
...
Minimizing $h(\bm{q})$ reduces the risk of joints reaching their technical limits.
Other, more sophisticated criteria are for example the quadratic \cite{HuoBar2005} or hyperbolic \cite{ZhuQuCaoYan2013} distance of from joint limits, stiffness stiffness \cite{GuoDonKe2015} or singularity avoidance via Frobenius-norm condition number \cite{ZhuQuCaoYan2013} or squared condition number \cite{LegerAng2016}.
The method can be used for serial link robots as well by removing all entries for the following legs from the formulas.
...
Further, using $\bm{\Psi}$/$\bm{\eta}$ as elaborated above allows for the first time to define differential kinematics specifically for 3T2R tasks in a general form.
...
=
\bm{\Phi}_{\bm{q}_{\mathrm{a}}} \dot{\bm{q}}_{\mathrm{a}} + \bm{\Phi}_{\bm{q}_{\mathrm{p}}} \dot{\bm{q}}_{\mathrm{p}} + \bm{\Phi}_{\bm{x}} \dot{\bm{x}} 
=
\bm{0}
\label{equ:constr_diff_qap}
\end{equation}
to distinguish active (``a'') and passive (``p'') joints. Reordering the equation leads to the full inverse differential kinematics
\begin{equation}
\bm{\Phi}_{\bm{x}} \dot{\bm{x}}
=
- 
\begin{pmatrix} \bm{\Phi}_{\bm{q}_{\mathrm{a}}} & \bm{\Phi}_{\bm{q}_{\mathrm{p}}}\end{pmatrix}
\begin{pmatrix}
\dot{\bm{q}}_{\mathrm{a}} \\
\dot{\bm{q}}_{\mathrm{p}} \\
\end{pmatrix}
=
\bm{\Phi}_{\mathrm{a}\mathrm{d}}
\begin{pmatrix}
\dot{\bm{q}}_{\mathrm{a}} \\
\dot{\bm{q}}_{\mathrm{p}} \\
\end{pmatrix},
...
\end{pmatrix}
=
-\bm{\Phi}_{\mathrm{a}\mathrm{d}}^{-1} \bm{\Phi}_{\bm{x}}
\dot{\bm{x}},
...
%
\begin{equation}
\bm{\Phi}_{\bm{q}_{\mathrm{a}}} \dot{\bm{q}}_{\mathrm{a}} 
=
- 
\begin{pmatrix} \bm{\Phi}_{\bm{x}} & \bm{\Phi}_{\bm{q}_{\mathrm{p}}} \end{pmatrix}
\begin{pmatrix}
\dot{\bm{x}} \\
\dot{\bm{q}}_{\mathrm{p}}
\end{pmatrix}
=
\bm{\Phi}_{\bm{x}\mathrm{d}}
\begin{pmatrix}
\dot{\bm{x}} \\
\dot{\bm{q}}_{\mathrm{p}}
\end{pmatrix},
...
\end{pmatrix}
=
-\bm{\Phi}_{\bm{x}\mathrm{d}}^{-1} \bm{\Phi}_{\bm{q}_{\mathrm{a}}}
\dot{\bm{q}}_{\mathrm{a}}.
\label{equ:full_diff_dk}
\end{equation}
%
By only selecting the first rows for $\dot{\bm{q}}_{\mathrm{a}}$ in (\ref{equ:full_diff_ik}) and for $\dot{\bm{x}}$ in (\ref{equ:full_diff_dk}), the well-known analytical Jacobian\footnote{This matrix is related to the time derivatives $\dot{\bm{x}}_{\mathrm{r}}$ of the platform Euler angles and is called ``design Jacobian'' in \cite{Gogu2008} and ``Euler
angles inverse jacobian matrix'' in \cite{Merlet2006} in contrast to the Jacobian related to angular velocities of the platform.} of the parallel robot \cite{Merlet2006,Gogu2008}, relating actuator velocities $\dot{\bm{q}}_{\mathrm{a}}$ and platform velocities $\dot{\bm{x}}$, can be obtained.
...
Sec.\,\ref{sec:appendix_eulerreciproc} contains a mathematical proof for the properties of reciprocal Euler angles in inverse kinematics, which is only outlined in \cite{1_SchapplerTapOrt2019}.
