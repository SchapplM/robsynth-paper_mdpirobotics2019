...
Further damping was introduced for the 3T2R case with task redundancy to reduce a ${\Delta}\bm{q}^{k}$ that would lead to overshoot over the joint limits with $K_\mathrm{Lim}(\bm{q})$. %, since the criterion $h_2$ from (\ref{equ:criterion2}) is only meaningfully defined within the joint limits for $q_{i,\mathrm{min}} < q_i < q_{i,\mathrm{max}}$.
...
Therefore, it was replaced by the $w_i$-weighted hyperbolic joint limit distance xxx
%
%\begin{equation}
%h_2(\bm{q})
%=
...
%\end{equation}
%
from \cite{ZhuQuCaoYan2013} with $n=6$ in this case and the corresponding gradient written element-wise as xxx
%\begin{equation}
%h_{2,\partial q_i}
%=
