...
The method allows to completely remove the redundant coordinate in 3T2R tasks and to solve the inverse kinematics for general serial and parallel robots with the \todo[color=red!40]{Terminology/ Hyphenation}gradient\replaced[id=Sp]{ }{-}descent algorithm.
...
Additionally, only members of class I provide redundancy which allows to use the additional DoF for performance \todo[color=green!40]{Language}optimizations\added[id=Sp]{,} e.g. to avoid singularities and to compensate the smaller workspace caused by the sixth leg.
...
%
For the bigger part of existing parallel robots, the passive joint coordinates can be eliminated analytically from the equations (\ref{equ:kinconstrAB}), e.g. by using the \todo[color=green!40]{Language}\replaced[id=Sp]{Euclidean}{Euclidian} distance for U\underline{P}S or R\underline{P}R leg chains or via trigonometry for \underline{R}RR\replaced[id=Sp]{ }{-}chains.
...
\begin{enumerate}
    ...
    \item The velocity-based theory of linear transformations used by \cite{Gogu2008} allows to determine the mobility of arbitrary parallel robots.
...
For 3T3R tasks, only Figure\,\ref{fig:pkm_constr_rot_lead}\,(c) represents a valid solution of the inverse kinematics. For 3T2R tasks, Figure\,\ref{fig:pkm_constr_rot_lead}\,(b) and (c) represent valid solutions.
...
This approach has already be used for many specific robots systems, as introduced in Section\,\ref{sec:intro}.
...
\added[id=Sp]{This allows to exploit the functional redundancy in 3T2R tasks also for trajectories $\bm{\eta}(t)$, $\dot{\bm{\eta}}(t)$ and not only for single poses $\bm{\eta}$. When solving the IKP for a trajectory, the IK on position level of Section\,\ref{sec:ZB_Anwendung_IK_grad} is only needed to correct linearization errors.}
...
The settings for the IK solver are similar as in Section\,\ref{sec:Ergebnisse_Seriell}, since both cases regard solving the IKP for a trajectory.
%The influence of the nullspace movement was increased with $K_\mathrm{N}=0.7$, since the effect is different for parallel robots
...
\todo[color=green!40]{Hyphenation}Additionally, the Denavit\added[id=Sp]{-}Hartenberg parameters were set to 50 different sets of uniformly distributed parameters between $0$ and $1$ meters or radians resulting to 2500 combinations for each of the 309 chains in total.
...
%The general kinematics of the industrial robot has 14 instead of the six kinematics parameters in Figure\,\ref{tab:mdh_industrialrobot}.
...
The generality of the approach allows to use it in a combined structural and dimensional synthesis sketched in Figure\,\ref{fig:structdimsynth}, extending the purely structural synthesis of parallel robot kinematics from \cite{Gogu2008,KongGos2005} to a dimensional synthesis of all structures as shown in \cite{RamirezKotOrt2015} for serial robots.
